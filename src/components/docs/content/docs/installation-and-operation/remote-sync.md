---
title: Remote sync
description: نسخه‌ کنترل داشبوردها، سؤال‌ها و مدل‌ها با Git و همگام‌سازی خودکار محتوا بین نمونه‌های توسعه و تولید.
---
...
Remote Sync این امکان را می‌دهد که محتوای تحلیلی را در یک متابیس توسعه (Development) بسازید و سپس از طریق Git آن را به یک نمونهٔ Production فقط‌خواندنی منتشر کنید.

کار با Remote Sync از طریق کالکشن‌های ویژه‌ای انجام می‌شود که به آن‌ها **synced collection** می‌گوییم. محتوای این کالکشن‌ها به‌طور خودکار نسخه‌گذاری می‌شود و می‌توانید آن را مستقیماً از رابط متابیس به ریپازیتوری Git خود push یا از آن pull کنید.

### مفاهیم کلیدی

**حالت‌های Development و Production**: Remote Sync دو حالت پیکربندی دارد که مشخص می‌کند هر نمونهٔ متابیس چطور با ریپازیتوری Git شما تعامل دارد:

- **Development mode**: برای ساخت و ویرایش محتوا. می‌توانید تغییرات را به Git push کنید، تغییرات دیگران را pull کنید، Branch بسازید و روی Branchهای فیچر کار کنید. چند نمونهٔ متابیس می‌توانند هم‌زمان در حالت Development باشند و هرکدام روی Branchهای متفاوت کار کنند.
- **Production mode**: برای سروکردن محتوای تأییدشده و فقط‌خواندنی. نمونه‌های Production فقط از Git (معمولاً از Branch اصلی مثل `main`) pull می‌کنند و اجازهٔ ویرایش مستقیم محتوای synced را نمی‌دهند. می‌توانید Auto‑sync را طوری تنظیم کنید که هر پنج دقیقه تغییرات تأییدشده را وارد Production کند.

**Synced collections**: کالکشن‌های ویژه‌ای که در Git دنبال می‌شوند. وقتی یک متابیس را در حالت Development به ریپو وصل می‌کنید، به‌طور پیش‌فرض یک کالکشن "Library" ساخته می‌شود. هر چیزی که داخل synced collection قرار بگیرد نسخه‌گذاری و با ریپازیتوری شما هماهنگ می‌شود.

**فایل‌های YAML سریال‌شده**: Remote Sync محتوای متابیس را به‌صورت فایل‌های YAML در Git ذخیره می‌کند. هر داشبورد، سؤال، مدل و سند به‌صورت یک فایل YAML نمایش داده می‌شود که می‌توان آن را در Pull Requestها بازبینی و مانند کد نسخه‌گذاری کرد. برای جزئیات بیشتر دربارهٔ فرمت YAML و ورک‌فلوهای خط فرمان، بخش [serialization](./serialization.md) را ببینید.

**ورک‌فلو مبتنی بر Branch**: در حالت Development می‌توانید Branch بسازید، بین Branchها سوئیچ کنید و تغییرات را commit کنید؛ همه از داخل UI متابیس. این قابلیت، ورک‌فلوهای فیچر‑Branch را ممکن می‌کند (توسعه روی Branch، بازکردن PR، بازبینی و در نهایت merge به Production).

### چرا از Remote Sync استفاده کنیم؟

Remote Sync برای تیم‌هایی طراحی شده که می‌خواهند یک **محیط تولید فقط‌خواندنی** داشته باشند؛ جایی که تیم مرکزی محتوا در محیط Development ساخته می‌شود و از طریق یک فرایند کنترل‌شده به Production می‌رسد.

- **Production فقط‌خواندنی**: کاربران در Production می‌توانند داشبوردها را ببینند و از آن‌ها استفاده کنند، اما نمی‌توانند محتوا را بسازند یا ویرایش کنند. تولید محتوا فقط در Development انجام می‌شود.
- **نسخه‌ کنترل برای محتوای جدید**: داشبوردها، سؤال‌ها و مدل‌های جدید از طریق Git و با تاریخچهٔ نسخه وارد Production می‌شوند.
- **بازبینی همتایان**: با Pull Request می‌توانید قبل از زنده‌شدن محتوا در Production، آن را بازبینی کنید.
- **استقرار کنترل‌شده**: با کنترل زمان merge به Branch اصلی، تعیین می‌کنید چه زمانی محتوای جدید در Production نمایش داده شود.

### Remote Sync در برابر Serialization

Remote Sync از همان فرمت سریال‌سازی زیرساختی قابلیت [CLI serialization متابیس](./serialization.md) استفاده می‌کند، اما هدف آن متفاوت است:

- **Remote Sync**: برای اضافه‌کردن محتوای جدید به Production فقط‌خواندنی با ورک‌فلو Git طراحی شده است.
- **Serialization**: ابزار خط فرمان برای Export و Import محتوای متابیس؛ مناسب برای مهاجرت محتوا و سناریوهای چندمحیطی پیچیده.

## راه‌اندازی Remote Sync

برای راه‌اندازی Remote Sync باید ادمین متابیس باشید.

1. [ریپازیتوری ذخیرهٔ محتوا را بسازید](#1-set-up-a-repository-to-store-your-content).
2. [یک personal access token برای توسعه بسازید](#2-create-a-personal-access-token-for-development).
3. [تفاوت حالت‌های Development و Production را درک کنید](#3-understand-development-and-production-modes).
4. [متابیس توسعه را به ریپو وصل کنید](#4-connect-your-development-metabase-to-your-repository).
5. [یک آیتم به synced collection خود اضافه کنید](#5-add-an-item-to-your-synced-collection).
6. [تغییرات را به ریپو push کنید](#6-push-your-changes-to-your-repository).
7. [یک personal access token برای Production بسازید](#7-create-a-personal-access-token-for-production).
8. [متابیس Production را به ریپو وصل کنید](#8-connect-your-production-metabase-to-your-repository).
