---
title: "قالب فایل تنظیمات متابیس"
---

# قالب فایل تنظیمات متابیس

می‌توانید این صفحهٔ مستندات را با رفتن به دایرکتوری ریشهٔ پروژهٔ متابیس و اجرای دستور زیر تولید کنید:

```sh
clojure -M:doc:ee config-template
```

این قالب، نمونه‌هایی از بخش‌های `database`، `user` و `settings` را برای [فایل تنظیمات](./config-file.md) فهرست می‌کند.
