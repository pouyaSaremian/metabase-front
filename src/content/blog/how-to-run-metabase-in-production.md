...
canonical: "how-to-run-metabase-in-production.html"
...
> We have a [new and improved version of this article](../learn/metabase-basics/administration/administration-and-operation/metabase-in-production.html).
...
- A server\(s\) to run the Metabase [Docker image or JAR](../start/oss/index.html) .
...
Metabase comes in two packages: a [Docker image](../docs/latest/installation-and-operation/running-metabase-on-docker.html) and a [JAR](../docs/latest/installation-and-operation/running-the-metabase-jar-file.html). Both provide the same experience: your users won’t know the difference.

You can’t make a bad call here: both options will keep your Metabase running smoothly, and both are [easy to scale](../learn/metabase-basics/administration/administration-and-operation/metabase-at-scale.html). And under the hood, they both run the Metabase JAR. The reason we provide both options is to give people flexibility when running Metabase in production.
...
You’ll need to [migrate the application database](../docs/latest/installation-and-operation/migrating-from-h2.html) that ships with Metabase \(an H2 database\) to a production SQL database. The application database stores all of your users, questions, dashboards, and other Metabase application data.
...
You’ll also be responsible for [backing up your application database](../docs/latest/installation-and-operation/backing-up-metabase-application-data.html) so you don’t lose your questions, dashboards, and other reporting data. Some cloud providers offer SQL databases as a service that can handle backups for you.
...
- Go through the [set up pages](../docs/latest/configuring-metabase/setting-up-metabase.html) .
- [Configure your settings](../docs/latest/configuring-metabase/start.html) .
- [Connect to your database\(s\)](../docs/latest/databases/connecting.html) .
- Confirm your [data model settings](../docs/latest/data-modeling/metadata-editing.html) .
...
- [Ask some questions](../docs/latest/questions/introduction.html) to check that you’ve set up your data model correctly.
- Create groups and [add some test users](../docs/latest/people-and-groups/managing.html) .
- Experiment with [data permissions](../learn/metabase-basics/administration/permissions/data-permissions.html) and [collection permissions](../learn/metabase-basics/administration/permissions/collection-permissions.html) using the test users.
- When you’re happy with you’re permissions, add real users and you’re off to the races.

## Further reading

- [Scaling Metabase](../learn/metabase-basics/administration/administration-and-operation/metabase-at-scale.html)
- [More about Metabase Cloud](Announcing-Metabase-Cloud.html)
